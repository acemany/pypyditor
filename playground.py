...
with open(gamedir/"coded.py", "r") as f:
    code_init = "from math import *; from pygame import draw"
    code_process = f.read()
    exec(code_init)
...
while True:
    ...
    WIN.blits(((FONT.render(excepp, 1, (255, 0, 0)), (0, 0)),
              *[(FONT.render(f"{j if j > 0 else '':>3}| {i}", 1, Ctxt2), (0, font_height*j))
                for j, i in enumerate(["", *[k for k in f"{code_textarea.manager.left}|{code_textarea.manager.right}".split("\n")]])]))
    display.flip()
